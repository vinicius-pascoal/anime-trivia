  ...
  const handleAnswer = (isCorrect: boolean, diff: string) => {
    ...
    if (questionCount + 1 >= maxQuestions) {
      setTimeout(() => {
        ...
        localStorage.setItem('anime_trivia_score', String(score + (isCorrect ? (diff === 'easy' ? 10 : diff === 'medium' ? 20 : 30) : 0)));
      }, 1000);
    } else {
      ...
      {gameState === 'playing' && (
        <>
          <div className="mb-4 flex items-center gap-4">
            ...
            <span className="text-lg">Pergunta: {questionCount + 1}/{maxQuestions}</span>
          </div>
